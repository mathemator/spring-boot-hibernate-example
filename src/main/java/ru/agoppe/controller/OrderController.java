...
import ru.agoppe.service.IOrderService;
...
public class OrderController {
	...
	@GetMapping("orders/departament/{departament}")
	public ResponseEntity<List<Order>> getOrdersByDepartament(@PathVariable("departament") String departament) {
		List<Order> list = articleService.getOrdersByDepartament(departament);
		return new ResponseEntity<List<Order>>(list, HttpStatus.OK);
	}
	@GetMapping("orders/master/{master}")
	public ResponseEntity<List<Order>> getOrdersByMaster(@PathVariable("master") String master) {
		List<Order> list = articleService.getOrdersByMaster(master);
		return new ResponseEntity<List<Order>>(list, HttpStatus.OK);
	}
	@PostMapping("order")
